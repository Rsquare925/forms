<template>
  <div class="form-container">
    ...
    <form class="form">
      ...
      <div>
        <button class="submit-btn w-[80%]" @click.prevent="submitClientMaster()">
          Submit
        </button>
      </div>
    </form>
...
function submitClientMaster() {
  ...
  if (
    ...
    validateFieldByLen(cinNo.value, cinNoEle, 21) &&
    validateFieldByLen(panNo.value, panNoEle, 10) &&
    validateFieldByLen(gstNo.value, gstNoEle, 15)
  ) {
    console.log("done");
  ...
  try {
    const res = await $fetch("/api/clientMaster", {
      ...
      body: {
        ...
        cinNo: cinNo.value,
        panNo: panNo.value,
        dueDays: dueDays.value ? Number(dueDays.value) : 0,
        openingBalance: openingBalance.value ? Number(openingBalance.value) : 0.0,
