    ...
    <div class="form-container">
        ...
        <form class="form p-4">
            <div>
                ...
                <input v-model="nameOfParty" type="text" maxlength="199" name="nameOfParty" id="nameOfParty" required>
            </div>

            <div class="">
                <input v-model="type" value="panNo" type="radio" id="pan" name="type" class="mr-2">
                <label for="pan">PAN</label><br>

                <input v-model="type" type="radio" id="aadhaar" name="type" value="aadhaarNo" class="mr-2">
                <label for="aadhaar">AADHAAR</label><br>

                <input v-model="type" type="radio" id="gst" name="type" value="gstNo" class="mr-2">
                <label for="gst">GST</label><br>

                <input v-model="type" type="radio" id="mobile" name="type" value="mobileNo" class="mr-2">
            ...
            <div v-show="type === 'aadhaarNo'">
                ...
                <input v-model="aadhaarNo" type="text" name="aadhaarNo" id="aadhaarNo" maxlength="12" required>
            </div>
            ...
            <div>
                ...
                <label>
                    <input @change="selectAll" class="font-bold p-3 mx-2 my-4  border-b-grey border-2 check-all" type="checkbox" >
                    Select All<br>
                </label>
                <label v-for="stn in stnNames">
                    <input class="font-bold p-3 mx-2 checkbox" type="checkbox" :value="stn.stnName" v-model="selectedStns">
                    {{ stn.stnName }}<br>
                </label>
                ...
                <button @click="toggleOverlay" class=" border-emerald-500 border-2 hover:border-blue-500 mt-5" id="select-branches-btn" type="button">SELECT BRANCHES</button>
            </div>

            <div>
                <button class="submit-btn w-[40%]" @click.prevent="submitGstMaster()">Submit</button>
            </div>
...
function submitGstMaster(){
    ...
    if(validateStringInput(nameOfParty, nameOfPartyEle, "Party's Name") && validateType() && validateBranches())
    {
        console.log("done");
        sendGstMasterData();
        // sendUserMasterData();
    ...
    try{
        const res = await $fetch('/api/gstMaster', {
            ...
            body: { 
                ...
                num: panNo.value ? panNo.value
                    : mobileNo.value ? mobileNo.value
                    : aadhaarNo.value ? aadhaarNo.value
                    : gstNo.value,
